...
public struct NextcloudContainer: Identifiable, Sendable {
    ...
    ///
    /// The Nextcloud server is reachable at `http://localhost:<port>`. The port is published on the loopback address only, so the server answers on this machine and not from other devices on the network.
    ///
    public let port: UInt

    ///
    /// The host port the websocket push endpoint is reachable on from this machine, or `nil` when ``NextcloudConfiguration/pushNotifications`` was not enabled.
    ///
